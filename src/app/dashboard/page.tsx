...
export default function DashboardPage() {
  ...
  async function loadAll() {
    ...
    // Load submissions for all public projects
    ...
    <div style={{ maxWidth: '820px', margin: '0 auto', padding: '32px 24px' }}>
      ...
      {showForm && (
        <form onSubmit={postProject} style={{ background: '#0f0f0f', border: '1px solid #2a2a2a', borderRadius: '14px', padding: '22px', marginBottom: '28px', display: 'flex', flexDirection: 'column', gap: '13px' }}>
          ...
          <div style={{ display: 'flex', background: '#111', border: '1px solid #222', borderRadius: '8px', padding: '3px', gap: '3px' }}>
            {(['public', 'private'] as const).map(v => (
              <button key={v} type="button" onClick={() => setVisibility(v)} style={{
                ...
                background: visibility === v ? '#1e1e1e' : 'transparent',
                color: visibility === v ? '#f0ece4' : '#555',
          ...
          {visibility === 'public' && <div style={{ fontSize: '12px', color: '#555' }}>Anyone on Project Lab can see and submit work for this project.</div>}
          {visibility === 'private' && <div style={{ fontSize: '12px', color: '#555' }}>Only the assigned individual will see this. Search by @username below.</div>}
          ...
          {/* Assignee (private only) */}
          {visibility === 'private' && (
            <input style={inp} placeholder="Assign to @username or email prefix" value={assignee} onChange={e => setAssignee(e.target.value)} />
          )}

          {postError && <div style={{ color: '#ff6b6b', fontSize: '13px', padding: '8px 12px', background: 'rgba(255,100,100,0.08)', borderRadius: '6px' }}>{postError}</div>}
